...
func ExtractDJVUMetadata(filePath string, cfg *config.Config) (author, title string, err error) {
	...
	if cfg.Debug {
		log.Printf("Djvused не сработал для %s: %v", filePath, err)
	}
	...
	if cfg.Debug {
		log.Printf("Djvudump не сработал: %v\n", err)
	}
	...
	if cfg.Debug {
		log.Printf("Djvutxt не сработал: %v\n", err)
	}
